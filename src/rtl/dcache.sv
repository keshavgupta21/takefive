...
module dcache(
    ...
    typedef enum logic [2:0] { REQ, WB, WB2, WAIT, FILL } state_t;
    ...
    takefive_pkg::cacheline_t [1:0] cache_rd_line, cache_wr_line;
    ...
    generate for(genvar i = 0; i < 2; i++) begin: gen_cache_mem
        ...
        assign cache_rd_line[i] = cache_mem_rd;
        assign cache_hit[i]     = cache_mem_vld[i][addr.idx] && (cache_rd_line[i].tag == addr.tag);

        always_comb begin
            cache_wr_line[i]                = cache_rd_line[i];
            cache_wr_line[i].words[addr.wo] = mem_req.data;
            cache_wr_line[i].dirty          = 1'b1;

            if ((state == REQ) && mem_req.vld && |cache_hit && mem_req.wen) begin
                cache_mem_we[i] = (hit_way == i[0]);
                cache_mem_wa[i] = addr.idx;
                cache_mem_wd[i] = cache_wr_line[i];
    ...
    always_ff @(posedge clk) begin
        ...
        mem_rsp.data <= rsp_fill ? dram_rsp.data[lat_addr.wo] : cache_rd_line[hit_way].words[addr.wo];
        mem_rsp.addr <= rsp_fill ? {lat_addr.tag, lat_addr.idx, lat_addr.wo, lat_addr.bo} : mem_req.addr;
    end

    // ---------------- State Machine ----------------
    logic line_needs_wb;
    assign line_needs_wb = cache_mem_vld[lru_way[addr.idx]][addr.idx] && cache_rd_line[lru_way[addr.idx]].dirty;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            lat_wen    <= mem_req.wen;
            mem_rdy    <= 0;
            state      <= line_needs_wb ? WB : WAIT;
            evict_line <= cache_rd_line[lru_way[addr.idx]];
            evict_way  <= lru_way[addr.idx];
        end else if (state == WB && dram_rdy) begin
            state <= WB2;
        end else if (state == WB2 && dram_rdy) begin
            state <= WAIT;
        end else if (state == WAIT && dram_rdy) begin
            state <= FILL;
        end else if (state == FILL && dram_rsp.vld) begin
